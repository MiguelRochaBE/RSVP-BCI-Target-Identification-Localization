  ...
  if (VirtualProtect(pFunction, size, PAGE_EXECUTE_READWRITE, &dwOldProtect)) {
    ...
    union Rel32Bytes {
      ...
      uint8_t bytes[4];
        ...
        ((int32_t)(uintptr_t)pDestFunction - (int32_t)(uintptr_t)pFunction) -
...
// We do not use SymbolLookup if ASAN is enabled because ASAN uses dbghelp with
// settings which are different from ours.
#if defined(OVR_OS_MS) && !IS_ASAN()
#if defined(OVR_CC_MSVC)
    ...
    if (requiredUTF8Length < OVR_ARRAY_COUNT(miniDumpFilePathW)) {
      ...
      if (hFile != INVALID_HANDLE_VALUE) {
        ...
        MinidumpCallbackContext minidumpCallbackContext = {
            minidumpInfoLevel, ::GetCurrentThreadId()};
        MINIDUMP_CALLBACK_INFORMATION minidumpCallbackInfo = {
            MinidumpFilter, &minidumpCallbackContext};
