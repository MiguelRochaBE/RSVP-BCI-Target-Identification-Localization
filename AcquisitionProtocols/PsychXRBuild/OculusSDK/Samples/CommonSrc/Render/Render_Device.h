...
struct RendererParams
{
    ...
    bool             SingleChannel;              // True if this is a grayscale, single-channel visual resource
    RendererParams() :
        RenderAPI(RenderAPI_None), SrgbBackBuffer(false), Resolution(0), DebugEnabled(false),
        GLMajorVersion(2), GLMinorVersion(1), GLCoreProfile(false), GLCompatibilityProfile(false), GLForwardCompatibleProfile(false), SingleChannel(false) {}
};
...
public:
    ...
    virtual ShaderFill *GetCubemapToEquirectFill() = 0;

    void ConvertCubemapToEquirect(Texture* cubemap, Texture* equirect);
    ...
    uint32_t ConvertFormatToMirrorOptions(uint64_t format)
    {
        ...
        if (format & Texture_MirrorForceSymmetricFov)
        {
            mirrorOptions |= ovrMirrorOption_ForceSymmetricFov;
        }
    ...
    virtual void SetScissors(int /* numScissors */, const Recti* /* scissors */) {OVR_ASSERT(false);}
