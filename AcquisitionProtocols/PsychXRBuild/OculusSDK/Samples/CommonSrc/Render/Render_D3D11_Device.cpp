...
static const char* CubemapToEquirectPixelShaderSrc =
    ...
    "  float theta = ov.TexCoord.y * PI;\n"
    "  float phi = ov.TexCoord.x * 2 * PI;\n"
    "  float3 vec = float3(0, 0, 0);\n"
    "  vec.x = sin(phi) * sin(theta);\n"
    "  vec.y = cos(theta);\n"
    "  vec.z = cos(phi) * sin(theta);\n"
    "  return Texture.Sample(Linear, vec);\n"
    "}\n";
    ...
    DepthBuffers()
{
...

    {
      ...
      CubemapToEquirectShaders->SetShader(VertexShaders[VShader_MVP]);
      CubemapToEquirectShaders->SetShader(PixelShaders[FShader_CubemapToEquirect]);
...
std::string floatToStrWithPrecision(float in, int precision)
{
...
std::string intToStrPadZeros(int in, int padWidth)
{
  std::ostringstream out;
  out.width(padWidth);
  out.fill('0');
...
bool RenderDevice::SaveCubemapTexture(Render::Texture * tex, Vector3f transl, const std::string& filePath, std::string* error)
{
    ...
    std::string counterStr = intToStrPadZeros(counter, 6);
    std::string cubemapRenderFilePath = directoryPath + "/Cubemap" + counterStr;
    for (int i = 0; i < 3; ++i)
    {
        float val = round(transl[i] * 1000.0f) / 1000.0f;
        std::string strVal = floatToStrWithPrecision(val, 3); // to millimeter precision
...
void RenderDevice::Render(const Matrix4f& matrix, Model* model)
{
    ...
    {
        Render(model->Fill ? model->Fill.GetRawRef() : DefaultFill,
