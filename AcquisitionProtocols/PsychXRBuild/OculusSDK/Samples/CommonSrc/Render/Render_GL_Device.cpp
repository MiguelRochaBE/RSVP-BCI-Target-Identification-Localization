...
// Converts a cubemap to an equirect
// TODO: Not implemented
static const char* CubemapToEquirectFragShaderSrc = "";
...
static bool CheckFramebufferStatus(GLenum target)
{
    ...
    case GL_FRAMEBUFFER_UNDEFINED:
        WriteLog("[RenderGLDevice] framebuffer not complete: GL_FRAMEBUFFER_UNDEFINED - returned if the specified framebuffer is the default read or draw framebuffer, but the default framebuffer does not exist.");
        break;
    case GL_FRAMEBUFFER_INCOMPLETE_ATTACHMENT:
        WriteLog("[RenderGLDevice] framebuffer not complete: GL_FRAMEBUFFER_INCOMPLETE_ATTACHMENT - returned if any of the framebuffer attachment points are framebuffer incomplete.");
        break;
    case GL_FRAMEBUFFER_INCOMPLETE_MISSING_ATTACHMENT:
        WriteLog("[RenderGLDevice] framebuffer not complete: GL_FRAMEBUFFER_INCOMPLETE_MISSING_ATTACHMENT - returned if the framebuffer does not have at least one image attached to it.");
        break;
    case GL_FRAMEBUFFER_INCOMPLETE_DRAW_BUFFER:
        WriteLog("[RenderGLDevice] framebuffer not complete: GL_FRAMEBUFFER_INCOMPLETE_DRAW_BUFFER - returned if the value of GL_FRAMEBUFFER_ATTACHMENT_OBJECT_TYPE is GL_NONE for any color attachment point named by GL_DRAW_BUFFERi.");
        break;
    case GL_FRAMEBUFFER_INCOMPLETE_READ_BUFFER:
        WriteLog("[RenderGLDevice] framebuffer not complete: GL_FRAMEBUFFER_INCOMPLETE_READ_BUFFER - returned if GL_READ_BUFFER is not GL_NONE and the value of GL_FRAMEBUFFER_ATTACHMENT_OBJECT_TYPE is GL_NONE for the color attachment point named by GL_READ_BUFFER.");
        break;
    case GL_FRAMEBUFFER_UNSUPPORTED:
        WriteLog("[RenderGLDevice] framebuffer not complete: GL_FRAMEBUFFER_UNSUPPORTED - returned if the combination of internal formats of the attached images violates an implementation-dependent set of restrictions.");
        break;
    case GL_FRAMEBUFFER_INCOMPLETE_MULTISAMPLE:
        WriteLog("[RenderGLDevice] framebuffer not complete: GL_FRAMEBUFFER_INCOMPLETE_MULTISAMPLE - returned if the value of GL_RENDERBUFFER_SAMPLES is not the same for all attached renderbuffers; if the value of GL_TEXTURE_SAMPLES is the not same for all attached textures; or, if the attached images are a mix of renderbuffers and textures, the value of GL_RENDERBUFFER_SAMPLES does not match the value of GL_TEXTURE_SAMPLES. also returned if the value of GL_TEXTURE_FIXED_SAMPLE_LOCATIONS is not the same for all attached textures; or, if the attached images are a mix of renderbuffers and textures, the value of GL_TEXTURE_FIXED_SAMPLE_LOCATIONS is not GL_TRUE for all attached textures.");
        break;
    case GL_FRAMEBUFFER_INCOMPLETE_LAYER_TARGETS:
        WriteLog("[RenderGLDevice] framebuffer not complete: GL_FRAMEBUFFER_INCOMPLETE_LAYER_TARGETS - returned if any framebuffer attachment is layered, and any populated attachment is not layered, or if all populated color attachments are not from textures of the same target.");
        break;
    default:
        WriteLog("[RenderGLDevice] framebuffer not complete: status 0x%x (unknown)", status);
        break;
    ...
    Lighting(NULL)
{
    ...
    {
      Ptr<ShaderSet> CubemapToEquirectShaders = *new ShaderSet();
      CubemapToEquirectShaders->SetShader(VertexShaders[VShader_MV]);
      CubemapToEquirectShaders->SetShader(FragShaders[FShader_CubemapToEquirect]);
      CubemapToEquirectFill = *new ShaderFill(CubemapToEquirectShaders);
    }
                      ...
                      const Matrix4f& matrix, int offset, int count, PrimitiveType rprim)
{
    ...
    if (shaders->ViewLoc >= 0) {
        ...
        glUniformMatrix4fv(shaders->ViewLoc, 1, 0, &transposed.M[0][0]);
    }
...
bool Shader::Compile(const char* src)
{
    ...
    if (!r)
    {
        ...
        if (msg[0])
            WriteLog("[RenderGLDevice] Compiling shader\n%s\nfailed: %s\n", src, msg);
        if (!r)
            return 0;
    }
    return 1;
...
bool ShaderSet::Link()
{
    ...
    if (!r)
    {
        ...
        WriteLog("[RenderGLDevice] Linking shaders failed: %s\n", msg);
        if (!r)
            return 0;
    }
    glUseProgram(Prog);
...
bool ShaderSet::SetUniform(const char* name, int n, const float* v)
{
    ...
    WriteLog("[RenderGLDevice] Warning: uniform %s not present in selected shader", name);
    return 0;
}
...
void DebugCallback::DebugCallbackInternal(Severity s, const char* pSource, const char* pType, GLuint id, const char* pSeverity, const char* message) const
{
    if(s >= MinLogSeverity)
    {
        WriteLog("[RenderGLDevice] {ERR-xxxx} [GL Error] %s %s %#x %s: %s", pSource, pType, id, pSeverity, message);
    }
    ...
    if(!Initialized)
    {
        ...
        if(!debugCallbackPrev) // If a callback isn't already registered...
        {
            ...
            if(GLE_KHR_debug)
            {
                ...
                if(err)
                {
                    WriteLog("[RenderGLDevice] glDebugMessageCallback error: %x (%d)\n", err, err);
                }
                ...
                if(err)
                {
                    WriteLog("[RenderGLDevice] GL_DEBUG_OUTPUT_SYNCHRONOUS error: %x (%d)\n", err, err);
                }
            ...
            else if(GLE_ARB_debug_output) // If KHR_debug wasn't found, try ARB_debug_output.
            {
                ...
                if(err)
                {
                    WriteLog("[RenderGLDevice] glDebugMessageCallbackARB error: %x (%d)\n", err, err);
                }
            ...
            else if(GLE_AMD_debug_output) // If ARB_debug_output also wasn't found, try AMD_debug_output.
            {
                ...
                if(err)
                {
                    WriteLog("[RenderGLDevice] glDebugMessageCallbackAMD error: %x (%d)\n", err, err);
                }
...
void GLVersionAndExtensions::ParseGLVersion()
{
    ...
    if (version)
    {
        WriteLog("[RenderGLDevice] GL_VERSION: %s", (const char*)version);
    ...
    {
        WriteLog("[RenderGLDevice] Warning: GL_VERSION was NULL\n");
    }
...
void GLVersionAndExtensions::ParseGLExtensions()
{
    ...
    else
    {
        ...
        if (!extensions)
        {
            ...
            WriteLog("[RenderGLDevice] Warning: GL_EXTENSIONS was NULL\n");
        }
        else
        {
            // Cannot print this to debug log: It's too long!
            //WriteLog("[RenderGLDevice] GL_EXTENSIONS: %s", (const char*)extensions);
        }
