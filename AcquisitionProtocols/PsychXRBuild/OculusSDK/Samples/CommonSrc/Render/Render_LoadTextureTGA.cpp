...
Texture* LoadTextureTgaEitherWay(RenderDevice* ren, File* f, int textureLoadFlags, unsigned char alpha, bool bottomUp)
{
    ...
    OVR_ASSERT ( ( ( bpp == 24 ) && ( descbyte == 0 ) ) || ( ( bpp == 32 ) && ( descbyte == 8 ) ) || ( ( bpp == 8 ) && ( descbyte == 0 ) ) );
    if ( ( descbyte & 0x10 ) != 0 )
    {
    ...
    case 3:
        ...
        case 8:
            for (int yc = height-1; yc >= 0; yc--)
            {
                ...
                {
                    f->Read(buf, 1);
                    imgdata[y*bpl+x*4+0] = buf[0];
                    imgdata[y*bpl+x*4+1] = buf[0];
                    imgdata[y*bpl+x*4+2] = buf[0];
                    imgdata[y*bpl+x*4+3] = alpha;
    ...
    uint64_t format = Texture_RGBA|Texture_GenMipmaps;
