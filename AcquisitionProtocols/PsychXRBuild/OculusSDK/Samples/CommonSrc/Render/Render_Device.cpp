...
namespace OVR { namespace Render {
    ...
    RenderDevice::RenderDevice(ovrSession session) :
        Session(session),
    ...
    Fill* RenderDevice::CreateTextureFill(Render::Texture* t, bool useAlpha, bool usePremult)
    {
        ...
        shaders->SetShader(LoadBuiltinShader(Shader_Fragment, usePremult ? FShader_AlphaPremultTexture : ( useAlpha ? FShader_AlphaTexture : FShader_Texture )));
        ...
        float x, float y, float size, Color c, const Matrix4f* view)
    {
        ...
        if(!font->fill)
        {
            font->fill = CreateTextureFill(Ptr<Texture>(
                *CreateTexture(Texture_R, font->twidth, font->theight, font->tex)), true, false);
        }
    ...
    void RenderDevice::FillTexturedRect(float left, float top, float right, float bottom, float ul, float vt, float ur, float vb, Color c, Ptr<Texture> tex, const Matrix4f* matrix, bool premultAlpha /*= false*/)
    {
        ...
        Fill *fill = GetTextureFill(tex, premultAlpha, premultAlpha);
        ...
        if (premultAlpha)
        {
            RenderWithAlpha(fill, pTextVertexBuffer, NULL, mat, 0, 6, Prim_Triangles);
        }
        else
        {
            Render(fill, pTextVertexBuffer, NULL, mat, 0, 6, Prim_Triangles);
        }
    }
        ...
        const Matrix4f* view)
    {
        ...
        m->AddVertex(left,  bottom,  0.5f, c, 0.0f, 0.0f);
        m->AddVertex(right, bottom,  0.5f, c, 1.0f, 0.0f);
        m->AddVertex(right, top,     0.5f, c, 1.0f, 1.0f);
        m->AddVertex(left,  top,     0.5f, c, 0.0f, 1.0f);
        m->AddTriangle(2,1,0);
        m->AddTriangle(0,3,2);
        m->Fill = image;
    ...
    void RenderDevice::ConvertCubemapToEquirect(Texture* cubemap, Texture* equirect)
    {
      AutoGpuProf prof(this, "Convert_Cubemap_To_Equirect");

      SetRenderTarget(equirect);
      SetViewport(0, 0, equirect->GetWidth(), equirect->GetHeight());
      SetProjection(Matrix4f::Identity());
      SetCullMode(Cull_Off);

      ShaderFill* cubemapToEquirectFill = GetCubemapToEquirectFill();
      cubemapToEquirectFill->SetTexture(0, cubemap);

      Matrix4f view = Matrix4f::Identity();
      RenderImage(-1, -1, 1, 1, cubemapToEquirectFill, 255, &view);
    }

    void RenderDevice::SetProjection(const Matrix4f& proj)
    {
        Proj = proj;
        SetWorldUniforms(Proj, GlobalTint);
    }

    void RenderDevice::SetGlobalTint(const Vector4f& globalTint)
    {
