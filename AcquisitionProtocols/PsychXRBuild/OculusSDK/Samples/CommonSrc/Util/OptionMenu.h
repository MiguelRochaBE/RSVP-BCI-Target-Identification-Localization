...
public:
    ...
    void NextValue(bool* pFastStep) override;
    void PrevValue(bool* pFastStep) override;
    // Set value from a string. Returns true on success.
    bool SetValue(const std::string& newVal) override;
    ...
    std::string ProcessShortcutKey(OVR::KeyCode key, bool shift) override;
    std::string ProcessShortcutButton(uint32_t buttonMask) override;
