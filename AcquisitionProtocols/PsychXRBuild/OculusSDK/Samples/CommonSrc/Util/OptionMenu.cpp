...
OptionMenuItem *OptionSelectionMenu::FindMenuItem(const std::string& menuItemLabel)
{
    ...
    {
        OptionMenuItem *subItem = Items[i].get();
...
bool OptionVar::SetValue(const std::string& newVal)
{
    bool success = false;
    switch (Type)
    {
    case Type_Enum:
    {
        ...
        {
            if ( 0 == OVR_stricmp (enumVal.Name.c_str(), newVal.c_str() ) )
            {
                *AsInt() = enumVal.Value;
                success = true;
                break;
        ...
        if (!success) // See if the newVal was an enum index instead of enum string.
        {
          int newIndex = std::stoi(newVal, nullptr, 10);

          for (const auto& enumVal : EnumValues)
          {
            if (newIndex == enumVal.Value)
            {
              *AsInt() = enumVal.Value;
              success = true;
              break;
    ...
    case Type_Bool:
        if ( ( 0 == OVR_stricmp (newVal.c_str(), "false" ) ) ||
             ( 0 == OVR_stricmp (newVal.c_str(), "0" ) ) ||
             ( 0 == OVR_stricmp (newVal.c_str(), "" ) ) )
        {
            *AsBool() = false;
        }
        else
...
{
    // MatchKey check for a match and executes the notification if so.
...
void OptionVar::Enumerate(OptionVar::VarSpecificationArray& varSpecificationArray,
                    const char* parentBranchPath) const
{
    VarSpecification spec;

    // Set the Type.
    spec.Type = Type;

    // Set the Name.
    spec.Name = parentBranchPath;
    if(!spec.Name.empty()) // In practice this will never be empty, but theoretically it could be.
        spec.Name += '.';
    spec.Name += Label;
    ...
    {
        case Type_Enum: {
            spec.EnumSpecification.EnumEntryArray = EnumValues;
            break;
        }

        case Type_Int: {
            spec.IntSpecification.Min = MinInt;
            spec.IntSpecification.Max = MaxInt;
            spec.IntSpecification.StepSize = StepInt;
            spec.IntSpecification.FormatString = FormatString ? FormatString : "%d";
            break;
        }

        case Type_Float: {
            spec.FloatSpecification.Min = MinFloat;
            spec.FloatSpecification.Max = MaxFloat;
            spec.FloatSpecification.StepSize = StepFloat;
            spec.FloatSpecification.FormatString = FormatString ? FormatString : "%f";
            break;
        }
    ...
    // We add shortcut information. Currently limited to the first keyboard shortcut,
    // as that's usually all there is.
    if(!ShortcutUp.Keys.empty())
        spec.ShortcutKeyUp = ShortcutUp.Keys.front();

    if(!ShortcutDown.Keys.empty())
        spec.ShortcutKeyDown = ShortcutDown.Keys.front();

    varSpecificationArray.emplace_back(spec);
}
...
void OptionSelectionMenu::Clear()
{
    Items.clear();
}
...
OptionSelectionMenu* OptionSelectionMenu::GetSubmenu()
{
    ...
    OptionSelectionMenu* submenu = static_cast<OptionSelectionMenu*>(Items[SelectedIndex].get());
    return submenu;
}


OptionSelectionMenu* OptionSelectionMenu::GetOrCreateSubmenu(const std::string& submenuName)
{
    for (size_t i = 0; i < Items.size(); i++)
    {
        if (!Items[i]->IsMenu())
        ...
        OptionSelectionMenu* submenu = static_cast<OptionSelectionMenu*>(Items[i].get());
...
void OptionSelectionMenu::Enumerate(OptionVar::VarSpecificationArray& varSpecificationArray,
                    const char* parentBranchPath) const
{
    // We have an array of OptionMenuItem (named Items) which is a base class that has only
    // two subclasses: a OptionVar and another OptionSelectionMenu. OptionVar is a leaf node
    // in this tree and every leaf in the tree, recusively, needs to be added to
    // varSpecificationArray. Some elements of the this array are OptionVars (leaf nodes) and
    // some are OptionMenuItems (branches). We add any leaf nodes to varSpecificationArray and
    // recursively call Enumerate on each of the branches. OptionVar implements this adding via
    // its own implementation of Enumerate, so the code below simply calls Enumerate on all Items.

    // Append our branch name our branch to the path
    std::string newParentBranchPath(parentBranchPath);
    if (!newParentBranchPath.empty())
        newParentBranchPath += '.';
    newParentBranchPath += Label;

    for(auto& item : Items)
    {
        if (item->IsMenu()) // Note that we can't simplify this with a base class Enumerate function, as that creates a chicken/egg problem.
            static_cast<OptionSelectionMenu*>(item.get())->Enumerate(varSpecificationArray, newParentBranchPath.c_str());
        else
            static_cast<OptionVar*>(item.get())->Enumerate(varSpecificationArray, newParentBranchPath.c_str());
    }
}


size_t OptionSelectionMenu::Enumerate(std::string& optionVarLines) const
{
    OptionVar::VarSpecificationArray varSpecificationArray;
    Enumerate(varSpecificationArray);

    std::string line;
    char buffer[2048];

    for(auto& spec : varSpecificationArray)
    {
        line.clear();

        switch (spec.Type)
        {
            case OptionVar::Type_Enum: {
                // We need to enumerate each item in turn.
                snprintf(buffer, sizeof(buffer), "\"%s\" type:enum, options:", spec.Name.c_str());
                line = buffer;

                for(auto& EnumEntry : spec.EnumSpecification.EnumEntryArray)
                {
                    snprintf(buffer, sizeof(buffer), " %s (%d),", EnumEntry.Name.c_str(), (int)EnumEntry.Value);
                    line += buffer;
                }
            ...
            case OptionVar::Type_Int: {
                char formatBuffer[64]; // We have to format the format buffer.
                snprintf(formatBuffer, sizeof(formatBuffer), "\"%%s\" type:int min:%s max:%s step:%s",
                    spec.IntSpecification.FormatString.c_str(),
                    spec.IntSpecification.FormatString.c_str(),
                    spec.IntSpecification.FormatString.c_str());

                snprintf(buffer, sizeof(buffer), formatBuffer,
                    spec.Name.c_str(), spec.IntSpecification.Min,
                    spec.IntSpecification.Max,
                    spec.IntSpecification.StepSize);

                line = buffer;
                break;
            }

            case OptionVar::Type_Float: {
                char formatBuffer[64]; // We have to format the format buffer.
                snprintf(formatBuffer, sizeof(formatBuffer), "\"%%s\" type:float min:%s max:%s step:%s",
                    spec.FloatSpecification.FormatString.c_str(),
                    spec.FloatSpecification.FormatString.c_str(),
                    spec.FloatSpecification.FormatString.c_str());

                snprintf(buffer, sizeof(buffer), formatBuffer,
                    spec.Name.c_str(), spec.FloatSpecification.Min, spec.FloatSpecification.Max,
                    spec.FloatSpecification.StepSize);

                line = buffer;
                break;
            }

            case OptionVar::Type_Bool: {
                snprintf(buffer, sizeof(buffer), "\"%s\" type:bool", spec.Name.c_str());
                line = buffer;
                break;
            }

            case OptionVar::Type_Trigger: {
                snprintf(buffer, sizeof(buffer), "\"%s\" type:trigger", spec.Name.c_str());
                line = buffer;
                break;
            }
        ...
        auto ShiftKeyUsageString = [](ShortcutKey::ShiftUsageType type) -> const char* {
            if(type == ShortcutKey::Shift_Ignore)
                return "Shift_Ignore";
            if(type == ShortcutKey::Shift_Modify)
                return "Shift_Modify";
            if(type == ShortcutKey::Shift_RequireOn)
                return "Shift_RequireOn";
            else
                return "Shift_RequireOff";
        };

        if(spec.ShortcutKeyUp.Key != Key_None)
        {
            // The OptionVar class uses ShortKeyUp to mean either step up (when stepping is present)
            // or simply setting the value (when it's not a stepping variable). So here we print
            // either shortcut or shortcut_up to indicate the actual usage.
            snprintf(buffer, sizeof(buffer), " shortcut%s: %d(%s)",
                (spec.ShortcutKeyDown.Key == Key_None) ? "" : "_up", // If there's a down key, this is an up key.
                spec.ShortcutKeyUp.Key,
                ShiftKeyUsageString(spec.ShortcutKeyUp.ShiftUsage));
            line += buffer;
        }

        if(spec.ShortcutKeyDown.Key != Key_None)
        {
            snprintf(buffer, sizeof(buffer), " shortcut_down: %d(%s)",
                spec.ShortcutKeyUp.Key,
                ShiftKeyUsageString(spec.ShortcutKeyUp.ShiftUsage));
            line += buffer;
        }

        // Append the line for this element
        optionVarLines += line;
        optionVarLines += '\n';

    } // for()

    return varSpecificationArray.size();
}
