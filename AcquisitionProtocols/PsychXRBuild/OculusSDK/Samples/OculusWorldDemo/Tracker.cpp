...
//----------------------------------------------------------------
void Tracker::Init(ovrSession Session, std::string mainFilePathNoExtension,
                   RenderDevice* pRender, bool SrgbRequested, bool AnisotropicSample, int Idx)
{
                   ...
                   bool Sitting, float ExtraSittingAltitude, Matrix4f * /*ViewFromWorld*/, int eye, ovrPosef * EyeRenderPose)
{
    ...
    ovrTrackerPose    trackerPose   = ovr_GetTrackerPose(Session, TrackerNum);
  ...
  bool              valid = (trackingState.StatusFlags & ovrStatus_PositionValid) ? true : false;
    ...
    {
        if (valid)
            globalTint = Vector4f(1, 0.75f, 0.80f, 1);  // Pink
        else
            globalTint = Vector4f(1, 0, 0, 1);  // Red
    }
