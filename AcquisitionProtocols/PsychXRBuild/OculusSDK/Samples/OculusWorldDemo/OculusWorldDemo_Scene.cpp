...
#define STR(x)  STR1(x)
    ...
    for (size_t i = 0; i < OVR_ARRAY_COUNT(baseDirectories); ++i)
    {
        ...
        WriteLog("[OculusWorldDemoScene] Trying to load the scene at: %s...", newPath.c_str());

        if (SysFile(newPath.c_str()).IsValid())
        {
            WriteLog("[OculusWorldDemoScene] Success loading %s", newPath.c_str());
            MainFilePath = newPath;
            return;
        }
    }

    WriteLog("[OculusWorldDemoScene] Unable to find any version of %s. Do you have your working directory set right?", WORLDDEMO_ASSET_FILE);
}
...
Fill* CreateTextureFill(RenderDevice* prender, const std::string& filename, unsigned int fillTextureLoadFlags, bool upside_down=false)
{
    ...
    {
        if (upside_down)
            imageTex = *LoadTextureTgaBottomUp(prender, imageFile, fillTextureLoadFlags, 255);
        else
...
void OculusWorldDemoApp::PopulateScene(const char *fileName)
{
    ...
    Ptr<Model> smallBlueCubeModel = *Model::CreateBox(Color(0, 0, 255, 255), Vector3f(0.0f, 0.0f, 0.0f), blockModelSizeVec);
    SmallBlueCube.World.Add(smallBlueCubeModel);
    ...
    for (int i = 0; i < TrackingCamera_Count; i++) {
      PositionalTrackers[i].Init(
          Session, mainFilePathNoExtension, pRender, SrgbRequested, AnisotropicSample, i);
    }

}

// We have a separate Hdcp texture load function because HDCP in the Oculus runtime might
// succeed handshaking after OWD is already running. So we want to be able to try reloading
// in case that has happened.
void OculusWorldDemoApp::LoadHdcpTexture()
{
  std::string mainFilePathNoExtension = MainFilePath;
  StripExtension(mainFilePathNoExtension);
  ...
  Ptr<File> imageFile = *new SysFile((mainFilePathNoExtension + "_Cockpit_Panel.tga").c_str());
  if (imageFile->IsValid()) {
    HdcpTexture = *LoadTextureTgaTopDown(pRender, imageFile, textureLoadFlags | TextureLoad_Hdcp, 255);
  }
}
...
void OculusWorldDemoApp::RenderAnimatedBlocks(CamRenderPoseEnum camNum, double appTime)
{
    ...
    case 2: // Vertical circle around a point.
        {
            ...
            for (int j = 0; j < 2; j++)
            {
                for (int i = 0; i < numBlocks; i++)
                {
                    ...
                    pBlockScene->Render(pRender, CamFromWorld[camNum] * translate * scaleUp);
                }
            }
        }
        break;
...
void OculusWorldDemoApp::RenderGrid(CamRenderPoseEnum camNum, Recti renderViewport)
{
    ...
    switch (camNum)
    {
    case CamRenderPose_Left:
    case CamRenderPose_Right:
    case CamRenderPose_ExternalCamera:
        cNormal = Color ( 0,   255, 0 );
        cSpacer = Color ( 255, 255, 0 );
        break;
    case CamRenderPose_Left1:
    case CamRenderPose_Right1:
        cNormal = Color ( 0, 255, 255 );
        cSpacer = Color ( 0, 255,   0 );
        break;
    case CamRenderPose_Left2:
    case CamRenderPose_Right2:
        cNormal = Color ( 255, 255, 0 );
        cSpacer = Color ( 0,   255, 0 );
        break;
    case CamRenderPose_Left3:
    case CamRenderPose_Right3:
        cNormal = Color ( 0, 255,   0 );
        cSpacer = Color ( 0, 255, 255 );
        break;
    default:
        OVR_ASSERT(!"write me" );
        break;
    ...
    switch (GridColor)
    {
    case OculusWorldDemoApp::GridColor_Default:
      // don't override
      break;
    case OculusWorldDemoApp::GridColor_White:
      cNormal = Color(255, 255, 255);
      cMid = Color(255, 255, 255);
      break;
    case OculusWorldDemoApp::GridColor_Red:
      cNormal = Color(255, 0, 0);
      cMid = Color(255, 0, 0);
      break;
    case OculusWorldDemoApp::GridColor_Green:
      cNormal = Color(0, 255, 0);
      cMid = Color(0, 255, 0);
      break;
    case OculusWorldDemoApp::GridColor_Blue:
      cNormal = Color(0, 0, 255);
      cMid = Color(0, 0, 255);
      break;
    case OculusWorldDemoApp::GridColor_Yellow:
      cNormal = Color(255, 255, 0);
      cMid = Color(255, 255, 0);
      break;
    case OculusWorldDemoApp::GridColor_Magenta:
      cNormal = Color(255, 0, 255);
      cMid = Color(255, 0, 255);
      break;
    case OculusWorldDemoApp::GridColor_Cyan:
      cNormal = Color(0, 255, 255);
      cMid = Color(0, 255, 255);
      break;
    default:
      OVR_FAIL();
      break;
    ...
    case Grid_Lens:
        {
            lineStep = 48;
            Vector2f rendertargetNDC = FovPort(CamFovPort[camNum]).TanAngleToRendertargetNDC(Vector2f(0.0f));
...
void OculusWorldDemoApp::RenderController(CamRenderPoseEnum camNum, const ovrPosef pose, bool rightSide) {

  pRender->SetCullMode(RenderDevice::Cull_Off);

  Matrix4f scaleUp = Matrix4f::Scaling(1.0f);
  // flip the pose around the x axis for the right side controller, since it is using the left
  // controller mesh.
  if (rightSide) {
    scaleUp = Matrix4f::Scaling(-1.0f, 1.0f, 1.0f);
  }

  Posef    worldPose = ThePlayer.VirtualWorldTransformfromRealPose(pose, TrackingOriginType);
  Matrix4f playerPose(worldPose);
  Matrix4f mat = CamFromWorld[camNum] * playerPose * scaleUp;
  ControllerScene.Render(pRender, mat);
  pRender->SetCullMode(RenderDevice::Cull_Back);
}

void OculusWorldDemoApp::RenderControllers(CamRenderPoseEnum camNum, ovrEyeType eyeNum )
{
    if (!HasInputState)
        return;
    OVR_UNUSED(eyeNum); // not used in all #ifdef blocks

    pRender->SetCullMode(RenderDevice::Cull_Off);

    {
      if (ConnectedControllerTypes & ovrControllerType_LTouch) {
        RenderController(camNum, HandPoses[ovrHand_Left], false);
      }
      if (ConnectedControllerTypes & ovrControllerType_RTouch) {
        RenderController(camNum, HandPoses[ovrHand_Right], true);
      }
    }
    ...
    if (ConnectedControllerTypes & ovrControllerType_Object0)
    {
        ...
        ControllerScene.Render(pRender, CamFromWorld[camNum] * playerPose * scaleUp);
