...
public:
   ...
   ~Player();
    ...
    // Frozen means that no movement is done. The user pose stays the same regardless of input.
    void SetUserFrozen(bool frozen) { UserFrozen = frozen; }
    bool GetUserFrozen() const { return UserFrozen; }
    ...
    void    AdjustBodyYaw(float degrees);
...
public:
    ...
    bool        UserFrozen; // If true then all pos/ori changes are ignored. For automation testing.

    // Where the avatar coordinate system (and body) is positioned and oriented in the virtual world
    // Modified by gamepad/mouse input. Expressed in degrees.
