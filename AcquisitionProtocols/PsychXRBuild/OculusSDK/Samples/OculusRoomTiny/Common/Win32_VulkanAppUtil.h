...
// Define this to use the legacy Vulkan interface
//#define USE_LEGACY_VULKAN_INTERFACE
...
public:
    ...
    bool QueryAvailableLayers(std::vector<VkLayerProperties>& availableLayers) {
    ...
    bool InitDevice(const char* appName, int vpW, int vpH, const VrApi& vrApi, bool windowed = true)
    {
        ...
        instInfo.pApplicationInfo = &appInfo;
            ...
            // The list of layers that will be enabled will be the intersection of requested
            // and available layers
        ...
        if (extensionNames[0] != '\0')
            strcat_s(extensionNames, " ");
        ...
        QueryAvailableLayers(layerProps);
        ...
        if (vrApi.CreateInstanceVk)
        {
            CHECKVK(vrApi.CreateInstanceVk(::vkGetInstanceProcAddr, &instInfo, nullptr, &instance));
        }
        else
        {
        ...
        if (vrApi.CreateDeviceVk != nullptr)
        {
            CHECKVK(vrApi.CreateDeviceVk(::vkGetInstanceProcAddr, physicalDevice, &deviceInfo, nullptr, &device));
        }
        else
        {
