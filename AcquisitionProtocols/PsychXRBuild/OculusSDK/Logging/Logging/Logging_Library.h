...
#define LOGGING_LOCATION_HASH()                                                                      \
  __pragma(warning(push)) __pragma(warning(disable : 4307)) ovrlog::LoggingFileLineHash(             \
      __FILE__,                                                                                      \
      0,                                                                                             \
      (uint64_t)((__LINE__ ^ LOGGING_FILE_LINE_FNV_HASH_OFFSET) * LOGGING_FILE_LINE_FNV_HASH_PRIME)) \
...
#if defined(_MSC_VER) // Microsoft doesn't support C99-Standard vsnprintf, so need to use
                      // _vscprintf.
