...
stim_order_files_path = "C:/Users/migue/OneDrive/Ambiente de Trabalho/Tese/AcquisitionProtocols/RealDroneImg_Protocol/Stim_Order_Files_Train"
...
EOG_text = visual.TextStim(win=win, name='EOG_text',
    text='Blink your eyes every 1 to 2 seconds.',
    font='Open Sans',
    pos=(0, 0), height=0.05, wrapWidth=None, ori=0.0, 
    color='white', colorSpace='rgb', opacity=None, 
    languageStyle='LTR',
...
routineForceEnded = not continueRoutine
while continueRoutine and routineTimer.getTime() < EOG_Screen_TIME:
    # get current time
    t = routineTimer.getTime()
    tThisFlip = win.getFutureFlipTime(clock=routineTimer)
    tThisFlipGlobal = win.getFutureFlipTime(clock=None)
    ...
    if EOG_text.status == STARTED:
        # is it time to stop? (based on global clock, using actual start)
        if tThisFlipGlobal > EOG_text.tStartRefresh + EOG_Screen_TIME -frameTolerance:
            # keep track of stop time/frame for later
    ...
    if stim_square_EOG.status == STARTED:
        # is it time to stop? (based on global clock, using actual start)
        if tThisFlipGlobal > stim_square_EOG.tStartRefresh + EOG_Screen_TIME-1-frameTolerance:
            # keep track of stop time/frame for later
...
if routineForceEnded:
    ...
    routineTimer.addTime(-EOG_Screen_TIME)
