...
file_path = 'C:/Users/migue/OneDrive/Ambiente de Trabalho/Tese/rsvp-target-localization-bci/AcquisitionProtocols/MADALENA_droneImages.xlsx'
stim_order_files_path = "C:/Users/migue/OneDrive/Ambiente de Trabalho/Tese/rsvp-target-localization-bci/AcquisitionProtocols/RealDroneImg_Protocol/Stim_Order_Files_Train"
...
routineForceEnded = not continueRoutine
while continueRoutine and routineTimer.getTime() < EMG_Screen_TIME:
    # get current time
    t = routineTimer.getTime()
    tThisFlip = win.getFutureFlipTime(clock=routineTimer)
    tThisFlipGlobal = win.getFutureFlipTime(clock=None)
    ...
    if EMG_text.status == STARTED:
        # is it time to stop? (based on global clock, using actual start)
        if tThisFlipGlobal > EMG_text.tStartRefresh + EMG_Screen_TIME -frameTolerance:
            # keep track of stop time/frame for later
    ...
    if stim_square_EMG.status == STARTED:
        # is it time to stop? (based on global clock, using actual start)
        if tThisFlipGlobal > stim_square_EMG.tStartRefresh + EMG_Screen_TIME-1-frameTolerance:
            # keep track of stop time/frame for later
...
if routineForceEnded:
    ...
    routineTimer.addTime(-EMG_Screen_TIME)
...
for thisTrial_block in trial_block:
    ...
    FIRST_N_NONTARGET_TRIALS = 5 # 1000 ms
    TARGET_SEPERATION = 3 # 600 ms target separation
    ...
    TARGET_SEPERATION = FIRST_N_NONTARGET_TRIALS
